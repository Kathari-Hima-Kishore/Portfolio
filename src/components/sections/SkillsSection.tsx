...
export const SkillsSection = memo(function SkillsSection({ isMobile = false }: SkillsSectionProps) {
    ...
    return (
        <section id="phase-2" className={`min-h-screen w-full relative flex items-center justify-center ${isMobile ? 'p-4' : 'p-4'}`}>

            {/* Heading - Absolute Top Left */}
            <div className={`absolute z-30 pointer-events-auto ${isMobile ? 'top-6 left-4' : 'top-10 left-6 md:top-16 md:left-16'}`}>
                <h2 className={`font-black text-white/80 drop-shadow-2xl tracking-tighter ${isMobile ? 'text-3xl' : 'text-4xl md:text-6xl'}`}>
                    Skills & Tools
                </h2>
            </div>

            {/* Mobile: Show Orbital Constellation */}
            {isMobile ? (
                ...
                <>
                    ...
                    <div className={`absolute z-30 pointer-events-auto ${isMobile ? 'bottom-6 right-4' : 'bottom-10 right-6 md:bottom-16 md:right-16'}`}>
                        <CubeButton
                        ...
                        <AnimatePresence>
                            {showSkillsList && (
                                <motion.div
                                    ...
                                    className={`w-full bg-surface/80 border border-white/10 rounded-3xl pointer-events-auto shadow-2xl backdrop-blur-xl ${isMobile ? 'p-4 max-w-full' : 'p-6 md:p-12 max-w-6xl'}`}
                                >
