...
export const TabletContactSection = memo(function TabletContactSection({ isPortrait = false }: TabletContactSectionProps) {
    ...
    return (
        <section id="phase-8" className={`min-h-screen flex items-center justify-center ${isPortrait ? 'p-4' : 'p-8'}`}>
            <motion.div
                initial="hidden"
                whileInView="visible"
                viewport={{ once: true, amount: 0.3 }}
                variants={sectionVariants}
                className={`max-w-4xl w-full pointer-events-auto ${isPortrait ? '' : 'px-4'}`}
            >
                {/* Header - positioned above form, left-aligned */}
                ...
                {/* Social Links - same style as PC/mobile */}
                <motion.div 
                    variants={sectionVariants}
